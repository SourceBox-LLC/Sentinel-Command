...
function SecurityProcedures() {
  return (
    <section className="docs-section" id="security-procedures">
      ...
      <h3>Compromised CameraNode API Key</h3>
      <p>If a CameraNode API key is compromised, an attacker could potentially push video segments to your storage:</p>
      <ol>
        ...
        <li><strong>Update the CameraNode</strong> — The CameraNode will disconnect. Re-run setup with the new API key.</li>
        ...
        <li><strong>Verify video integrity</strong> — If you suspect tampered footage, check your CameraNode logs for upload activity you don't recognize.</li>
      </ol>
      ...
      <ol>
        ...
        <li><strong>Rotate CameraNode keys</strong> — If the user had <code>manage_cameras</code> permission, rotate all node API keys from <Link to="/settings">Settings</Link>.</li>
      ...
      <ol>
        ...
        <li><strong>Cut the camera off-hours</strong> — Live streaming runs whenever the CameraNode is running; there's no built-in "viewing window" today. If you only need the camera live during certain hours, the right tool is to power the CameraNode itself on a schedule (PoE switch, smart plug, systemd timer) or to physically disable the USB camera when it's not needed. (Scheduled recording controls when CameraNode writes durable footage, not who can watch the live feed.)</li>
      </ol>

      <h3>General Security Best Practices</h3>
      <ul>
        <li><strong>Rotate keys regularly</strong> — Rotate CameraNode and MCP API keys periodically, even without an incident.</li>
        ...
        <li><strong>Keep CameraNode updated</strong> — Always run the latest version for security patches.</li>
