...
> **Brand-history note for grep-discoverability:** the product has carried three names — `OpenSentry` (early), `SourceBox Sentry` (mid), and `Sentinel by SourceBox` (current, from May 2026 onward). The `Sentinel AI` name is reserved specifically for the AI-agent feature. The Command Center GitHub repo was renamed `OpenSentry-Command` → `Sentinel-Command` in May 2026; the old URL still resolves via GitHub's auto-redirect, so any hardcoded reference to the old name in a release artifact / cached doc / external bookmark continues to work. The CloudNode repo (`opensentry-cloud-node`) was deliberately **not** renamed — its name is referenced by both `install.sh` and the MSI download URL that customers may have bookmarked. Identifiers preserved verbatim across the entire rebrand (do not rename without a migration plan): the binary `sourcebox-sentry-cloudnode`, the env-var prefix `SOURCEBOX_SENTRY_*`, Windows install path `C:\ProgramData\SourceBoxSentry\`, the AES key-derivation domain string `opensentry-cloudnode-machine-id-v2` (see CloudNode `database.rs::KEY_DOMAIN_V2`), and the production hostname `opensentry-command.fly.dev` (tied to the Fly app, decoupled from the repo rename).
