...
- `DATABASE_URL` — defaults to `sqlite:///./sentinel.db`. **One codebase, two engines**, selected by this URL's scheme (see `app/core/database.py`, which branches on it):
  - **Hosted production runs Postgres** (`postgresql+psycopg://…`, on the shared `sentinel-sync-db` cluster). Set as a Fly *secret*, not in `fly.toml`, because it carries a password. ⚠️ That secret is a **cluster-wide superuser** credential, not a scoped one — `fly postgres attach` creates every role `SUPERUSER`, so it can read and write Sync-Service's and License-Service's databases too. Verified, and open; the remediation is in `docs/runbooks/DISASTER_RECOVERY.md`. Default QueuePool with `pool_pre_ping=True` — `NullPool` would mean a fresh TCP+auth round trip per request now that the database is across a network. Note the `+psycopg` driver suffix: `fly postgres attach` emits a bare `postgres://` URL, which SQLAlchemy would route to psycopg2 (not installed).
