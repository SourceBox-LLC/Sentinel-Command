...
- `DATABASE_URL` — defaults to `sqlite:///./sentinel.db`. **One codebase, two engines**, selected by this URL's scheme (see `app/core/database.py`, which branches on it):
  - **Hosted production runs Postgres** (`postgresql+psycopg://…`) on its **own dedicated cluster, `sentinel-command-db`**. Set as a Fly *secret*, not in `fly.toml`, because it carries a password. Each hosted service has its own cluster since 2026-09-07 (`sentinel-command-db`, `sentinel-license-db`, `sentinel-sync-db`) — before that all three shared one, where `fly postgres attach`'s default `SUPERUSER` roles meant any one service's credential could read and write the others' data. Splitting them fixed that: verified that these credentials can no longer reach another service's cluster. The role is still a superuser **on its own cluster**, which is a far smaller concern — it is that service's own data — but it does mean `DATABASE_URL` is an admin credential for this database, not a scoped one. Default QueuePool with `pool_pre_ping=True` — `NullPool` would mean a fresh TCP+auth round trip per request now that the database is across a network. Note the `+psycopg` driver suffix: `fly postgres attach` emits a bare `postgres://` URL, which SQLAlchemy would route to psycopg2 (not installed).
  - **Self-hosted runs SQLite** — NullPool, WAL, `synchronous=NORMAL`, `check_same_thread=False`. These are pysqlite-only: the PRAGMA handler is registered *conditionally* and the connect args are applied only for SQLite, because `PRAGMA` is a syntax error on Postgres and psycopg rejects those kwargs. Since the hosted deployment moved to Postgres, this path is now **exclusively** self-hosted — it has no production traffic behind it any more, so treat changes here as changes to someone else's install.
    - ⚠️ **Known inconsistency:** `connect_args` sets pysqlite's `timeout=30` (30 s) but the connect-time PRAGMA then sets `busy_timeout=5000` (5 s), which silently wins — the PRAGMA runs right after connect. So the effective busy timeout is **5 s**, not the 30 s the connect arg implies. Sentinel-License-Service resolves the same pair the other way (`busy_timeout=30000`, with a comment saying so). Unresolved which is intended here: 5 s arguably suits Command Center's bursty HLS segment uploads, where failing fast beats blocking a writer for 30 s. Decide and make the two lines agree.
